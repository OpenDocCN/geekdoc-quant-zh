...
交易员通过导入常用别名为'pd'的 pandas 库来开始他们的分析：
...
首先，我们的交易员导入 matplotlib 和 pandas，以确保他们拥有全面的数据处理和可视化工具：
...
这些只是 SciPy 在金融分析中的潜在应用的一些例子。在本书中，我们将深入探讨 SciPy 的广泛应用，从市场数据过滤的信号处理到定量模型构建中的积分和微分。SciPy 不仅仅是一个库；它是金融分析师工具箱中的科学探索灯塔。
